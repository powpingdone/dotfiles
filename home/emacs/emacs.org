  ...
  (use-package org
    ...
    (setq org-enforce-todo-dependencies t
	  ...
	  org-hide-leading-stars t
          ...
          org-todo-keywords '(
            (sequence "TODO(t!)"       ; newly created / low-priority tasks
                      "NOW(n!)"        ; what I need to work on now
                      "WAITING(w@)"    ; waiting on other things to happen
                      "MONITOR(m!)"    ; continually checkup on this
                      "|"
                      "DONE(d@)"       ; no further action needed
                      "REMOVED(r@)")   ; dead project

            (sequence "EVENT(e!)"      ; new/generic event
                      "PHONE(p@)"      ; phone call to be scheduled
                      "ONLINE(o@)"     ; online service (teams, zoom) to call
                      "GOTO(g@)"       ; physical location
                      "|"
                      "CANCELLED(c@)"  ; dead event
                      "VISITED(v!)")   ; event finished

            (sequence "IDEA"           ; new idea
                      "|"
                      "QUASH(Q@)"      ; bad idea
                      "CONSIDER(C@)")) ; good idea
      
          org-todo-state-tags-triggers '(
            (done ("DEAD" . t) ("WORKABLE" . nil))
            ("TODO" 
              ("WORKABLE" . t) ("EVENT" . nil) ("ACTIVE" . nil) ("SLEEP" . nil))
            ("NOW" 
              ("ACTIVE" . t) ("SLEEP" . nil))
            ("WAITING" 
              ("SLEEP" . t) ("ACTIVE" . nil) ("WORKABLE" . nil))
            ("MONITOR"
              ("WORKABLE" . t) ("SLEEP" . nil) ("ACTIVE" . nil))
;; unfinished, TODO:  DONE, REMOVED, CANCELLED, VISITED, QUASH, CONSIDER

            ("EVENT" 
              ("EVENT" . t) ("WORKABLE" . nil))
          ...
          org-todo-keyword-faces '((
            ...
            ("NOW" :foreground (doom-color 'red) :weight 'ultra-bold)
            ("WAITING" :foreground (doom-color 'teal))
            ("MONITOR" :foreground (doom-color 'orange))
            ("DONE" :foreground (doom-color 'comments))
            ("REMOVED" :foreground (doom-color 'comments))

            ("EVENT" :foreground (doom-color 'blue))
            ("PHONE" :foreground (doom-color 'cyan) :underline t)
            ("ONLINE" :foreground (doom-color 'cyan) :underline t)
            ("GOTO" :foreground (doom-color 'cyan) :underline t)
            ("CANCELLED" :foreground (doom-color 'dark-cyan))
            ("VISITED" :foreground (doom-color 'dark-cyan))
           
            ("IDEA" :inherit 'org-todo)
            ("QUASH" :foreground (doom-color 'comments))
            ("CONSIDER" :foreground (doom-color 'violet))))

          org-capture-templates '(
            ("i" "idea" entry (file "ideas.org")
               "* [#9] %? :IDEA:\n%t\n")
            ("t" "generic task" entry (file "")
               "* TODO [#4] %?\n%u\n%a\n")
            ("w" "work task" entry (file "")
               "* QUEUE [#3] %?\n%u\n%a")
            ("s" "school task" entry (file "")
               "* QUEUE [#2] %?\n%u\n%a")
            ("p" "PRIORITY TASK" entry (file "")
               "* NOW [#1] %?\n%u\n%a")
            ("H" "happening" entry (file "")
               "* EVENT [#4] %?\nSCHEDULED: %T\n%u\n%a\n")
