...
Add modes that override. Report bug that forces ~require~ ing the lsp-mode modules.
...
#+begin_src emacs-lisp
    (use-package lsp-mode
      ...
      ;; init on modes
      (dolist (hook
          	   '(python-ts-mode python-mode nix-mode))
        (add-hook (intern
          	       (concat (symbol-name hook) "-hook")) 'lsp-deferred))
      :hook
      (lsp-before-initialize . direnv-update-environment)
      (lsp-mode . lsp-enable-which-key-integration)
      (lsp-mode . (lambda () (leader-key "lv" '(lsp-command-map :wk "lsp-mode"))))
      :commands (lsp lsp-deferred)
      :config
      ;; Show that this is apparently required to get this working...
      (dolist (name
    	   '(completion
    	     diagnostics
    	     dired
    	     ...
    	     protocol
    	     semantic-tokens))
        (require (intern (concat "lsp-" (symbol-name name))))))
